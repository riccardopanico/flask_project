...
class ApiOracleManager:
    ...
    def _get_access_token(self):
        ...
        current_app.logger.debug("into _get_access_token")
        ...
        try:
            ...
            current_app.logger.debug(f"Token response: {response.json().get('access_token')}")
            ...
            current_app.logger.error(f"Errore durante la richiesta del token: {e}")
    ...
    def call(self, url, params=None, method='GET'):
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 201:
                ...
                current_app.logger.error(f"Errore nella chiamata API: {response.text}")
                return {'success': False, 'status': response.status_code, 'error': response.text}

        except RequestException as e:
            current_app.logger.error(f"Errore durante la chiamata API: {e}")
